<script lang="ts">
	...
	function play_show(video_or_tree: (Block & Video) | Tree) {
		const local_video: Block & Video = is_tree(video_or_tree) ? video_or_tree.node : video_or_tree;
		player.start_show(local_video as Video);
	}

	function is_tree(video_or_tree: (Block & Video) | Tree): video_or_tree is Tree {
		return 'node' in video_or_tree;
	}
</script>
