...
const new_player_state = () => {
	...
	function save_position(position?: number) {
		...
		if (current_state.audio && current_state?.current_show?.id) {
			localStorage.setItem(
				`last_played_position_${current_state.current_show.id}`,
	...
	async function load_show(
		...
		const local_storage_episode_state = localStorage.getItem(
			`last_played_position_${incoming_show.id}`
		);
	...
	return {
		...
		async start_show(requested_show: Show, play_from_position?: number) {
			...
			try {
				// Analytics - Sentry metrics API removed in recent versions
				// Sentry.metrics.increment('video_start', 1, { tags: { video: incoming_show.id } });
				// Sentry.metrics.increment('all_video_start', 1);
			...
			update((state) => {
				if (state.audio) {
					if (state.current_show) {
						localStorage.setItem(
							`last_played_position_${state.current_show.id}`,
